...
function generateTrialCombinations() {
    ...
    // Each sure paired with each unique pair of gambles
...
async function saveDataToDropbox() {
    ...
    if (!isOnline) {
        ...
        saveDataLocally(subjectName, "sure_vs_2gambles", params, experimentData, currentTrial, currentBlock);
        return;
    }
    ...
    try {
        ...
        const filename = `/mkturkfolders/datafiles/$${subject}/$${subject}_sure_vs_2gambles_${timestamp}.json`;
...
async function presentThreeChoices(sureStimulus, gamble1Stimulus, gamble2Stimulus) {
    return new Promise((resolve) => {
        clearDisplay();
        
        // Randomize positions
        const stimuli = [
            { stimulus: sureStimulus, type: 'sure' },
...
async function runTrial() {
    ...
    const response = await presentThreeChoices(
        trialData.sureStimulus,
    ...
    let sureValue = getSureRewardValue(trialData.sureStimulus.path);
    
    if (response.timeout) {
        console.log('Timeout - no response');
    } else {
        if (response.chosenType === 'sure') {
    ...
    experimentData.push({
        ...
        sureStimulus: trialData.sureStimulus.path,
        ...
        surePosition: Object.keys(response.positionMap).find(k => response.positionMap[k].type === 'sure'),
